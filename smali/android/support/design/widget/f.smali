.class final Landroid/support/design/widget/f;
.super Ljava/lang/Object;


# static fields
.field private static final k:Z

.field private static final l:Landroid/graphics/Paint;


# instance fields
.field private A:Z

.field private B:Z

.field private C:Landroid/graphics/Bitmap;

.field private D:Landroid/graphics/Paint;

.field private E:F

.field private F:F

.field private G:F

.field private H:F

.field private I:[I

.field private J:Z

.field private final K:Landroid/text/TextPaint;

.field private L:Landroid/view/animation/Interpolator;

.field private M:F

.field private N:F

.field private O:F

.field private P:I

.field private Q:F

.field private R:F

.field private S:F

.field private T:I

.field a:F

.field b:I

.field c:I

.field d:F

.field e:F

.field f:Landroid/content/res/ColorStateList;

.field g:Landroid/graphics/Typeface;

.field h:Landroid/graphics/Typeface;

.field i:Ljava/lang/CharSequence;

.field j:Landroid/view/animation/Interpolator;

.field private final m:Landroid/view/View;

.field private n:Z

.field private final o:Landroid/graphics/Rect;

.field private final p:Landroid/graphics/Rect;

.field private final q:Landroid/graphics/RectF;

.field private r:Landroid/content/res/ColorStateList;

.field private s:F

.field private t:F

.field private u:F

.field private v:F

.field private w:F

.field private x:F

.field private y:Landroid/graphics/Typeface;

.field private z:Ljava/lang/CharSequence;


# direct methods
.method static constructor <clinit>()V
    .locals 2

    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v1, 0x12

    if-ge v0, v1, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    sput-boolean v0, Landroid/support/design/widget/f;->k:Z

    return-void
.end method

.method public constructor <init>(Landroid/view/View;)V
    .locals 1

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    const/16 v0, 0x10

    iput v0, p0, Landroid/support/design/widget/f;->b:I

    iput v0, p0, Landroid/support/design/widget/f;->c:I

    const/high16 v0, 0x41700000    # 15.0f

    iput v0, p0, Landroid/support/design/widget/f;->d:F

    iput v0, p0, Landroid/support/design/widget/f;->e:F

    iput-object p1, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    new-instance p1, Landroid/text/TextPaint;

    const/16 v0, 0x81

    invoke-direct {p1, v0}, Landroid/text/TextPaint;-><init>(I)V

    iput-object p1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    new-instance p1, Landroid/graphics/Rect;

    invoke-direct {p1}, Landroid/graphics/Rect;-><init>()V

    iput-object p1, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    new-instance p1, Landroid/graphics/Rect;

    invoke-direct {p1}, Landroid/graphics/Rect;-><init>()V

    iput-object p1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    new-instance p1, Landroid/graphics/RectF;

    invoke-direct {p1}, Landroid/graphics/RectF;-><init>()V

    iput-object p1, p0, Landroid/support/design/widget/f;->q:Landroid/graphics/RectF;

    return-void
.end method

.method private static a(FFFLandroid/view/animation/Interpolator;)F
    .locals 0

    if-eqz p3, :cond_0

    invoke-interface {p3, p2}, Landroid/view/animation/Interpolator;->getInterpolation(F)F

    move-result p2

    :cond_0
    invoke-static {p0, p1, p2}, Landroid/support/design/widget/a;->a(FFF)F

    move-result p0

    return p0
.end method

.method private static a(IIF)I
    .locals 5

    const/high16 v0, 0x3f800000    # 1.0f

    sub-float/2addr v0, p2

    invoke-static {p0}, Landroid/graphics/Color;->alpha(I)I

    move-result v1

    int-to-float v1, v1

    mul-float/2addr v1, v0

    invoke-static {p1}, Landroid/graphics/Color;->alpha(I)I

    move-result v2

    int-to-float v2, v2

    mul-float/2addr v2, p2

    add-float/2addr v1, v2

    invoke-static {p0}, Landroid/graphics/Color;->red(I)I

    move-result v2

    int-to-float v2, v2

    mul-float/2addr v2, v0

    invoke-static {p1}, Landroid/graphics/Color;->red(I)I

    move-result v3

    int-to-float v3, v3

    mul-float/2addr v3, p2

    add-float/2addr v2, v3

    invoke-static {p0}, Landroid/graphics/Color;->green(I)I

    move-result v3

    int-to-float v3, v3

    mul-float/2addr v3, v0

    invoke-static {p1}, Landroid/graphics/Color;->green(I)I

    move-result v4

    int-to-float v4, v4

    mul-float/2addr v4, p2

    add-float/2addr v3, v4

    invoke-static {p0}, Landroid/graphics/Color;->blue(I)I

    move-result p0

    int-to-float p0, p0

    mul-float/2addr p0, v0

    invoke-static {p1}, Landroid/graphics/Color;->blue(I)I

    move-result p1

    int-to-float p1, p1

    mul-float/2addr p1, p2

    add-float/2addr p0, p1

    float-to-int p1, v1

    float-to-int p2, v2

    float-to-int v0, v3

    float-to-int p0, p0

    invoke-static {p1, p2, v0, p0}, Landroid/graphics/Color;->argb(IIII)I

    move-result p0

    return p0
.end method

.method private static a(FF)Z
    .locals 0

    sub-float/2addr p0, p1

    invoke-static {p0}, Ljava/lang/Math;->abs(F)F

    move-result p0

    const p1, 0x3a83126f    # 0.001f

    cmpg-float p0, p0, p1

    if-gez p0, :cond_0

    const/4 p0, 0x1

    return p0

    :cond_0
    const/4 p0, 0x0

    return p0
.end method

.method private static a(Landroid/graphics/Rect;IIII)Z
    .locals 1

    iget v0, p0, Landroid/graphics/Rect;->left:I

    if-ne v0, p1, :cond_0

    iget p1, p0, Landroid/graphics/Rect;->top:I

    if-ne p1, p2, :cond_0

    iget p1, p0, Landroid/graphics/Rect;->right:I

    if-ne p1, p3, :cond_0

    iget p0, p0, Landroid/graphics/Rect;->bottom:I

    if-ne p0, p4, :cond_0

    const/4 p0, 0x1

    return p0

    :cond_0
    const/4 p0, 0x0

    return p0
.end method

.method static a(Landroid/graphics/Typeface;Landroid/graphics/Typeface;)Z
    .locals 1

    if-eqz p0, :cond_0

    invoke-virtual {p0, p1}, Landroid/graphics/Typeface;->equals(Ljava/lang/Object;)Z

    move-result v0

    if-eqz v0, :cond_1

    :cond_0
    if-nez p0, :cond_2

    if-eqz p1, :cond_2

    :cond_1
    const/4 p0, 0x1

    return p0

    :cond_2
    const/4 p0, 0x0

    return p0
.end method

.method private b(F)V
    .locals 6

    iget-object v0, p0, Landroid/support/design/widget/f;->q:Landroid/graphics/RectF;

    iget-object v1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    iget v1, v1, Landroid/graphics/Rect;->left:I

    int-to-float v1, v1

    iget-object v2, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    iget v2, v2, Landroid/graphics/Rect;->left:I

    int-to-float v2, v2

    iget-object v3, p0, Landroid/support/design/widget/f;->j:Landroid/view/animation/Interpolator;

    invoke-static {v1, v2, p1, v3}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v1

    iput v1, v0, Landroid/graphics/RectF;->left:F

    iget-object v0, p0, Landroid/support/design/widget/f;->q:Landroid/graphics/RectF;

    iget v1, p0, Landroid/support/design/widget/f;->s:F

    iget v2, p0, Landroid/support/design/widget/f;->t:F

    iget-object v3, p0, Landroid/support/design/widget/f;->j:Landroid/view/animation/Interpolator;

    invoke-static {v1, v2, p1, v3}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v1

    iput v1, v0, Landroid/graphics/RectF;->top:F

    iget-object v0, p0, Landroid/support/design/widget/f;->q:Landroid/graphics/RectF;

    iget-object v1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    iget v1, v1, Landroid/graphics/Rect;->right:I

    int-to-float v1, v1

    iget-object v2, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    iget v2, v2, Landroid/graphics/Rect;->right:I

    int-to-float v2, v2

    iget-object v3, p0, Landroid/support/design/widget/f;->j:Landroid/view/animation/Interpolator;

    invoke-static {v1, v2, p1, v3}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v1

    iput v1, v0, Landroid/graphics/RectF;->right:F

    iget-object v0, p0, Landroid/support/design/widget/f;->q:Landroid/graphics/RectF;

    iget-object v1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    iget v1, v1, Landroid/graphics/Rect;->bottom:I

    int-to-float v1, v1

    iget-object v2, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    iget v2, v2, Landroid/graphics/Rect;->bottom:I

    int-to-float v2, v2

    iget-object v3, p0, Landroid/support/design/widget/f;->j:Landroid/view/animation/Interpolator;

    invoke-static {v1, v2, p1, v3}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v1

    iput v1, v0, Landroid/graphics/RectF;->bottom:F

    iget v0, p0, Landroid/support/design/widget/f;->u:F

    iget v1, p0, Landroid/support/design/widget/f;->v:F

    iget-object v2, p0, Landroid/support/design/widget/f;->j:Landroid/view/animation/Interpolator;

    invoke-static {v0, v1, p1, v2}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v0

    iput v0, p0, Landroid/support/design/widget/f;->w:F

    iget v0, p0, Landroid/support/design/widget/f;->s:F

    iget v1, p0, Landroid/support/design/widget/f;->t:F

    iget-object v2, p0, Landroid/support/design/widget/f;->j:Landroid/view/animation/Interpolator;

    invoke-static {v0, v1, p1, v2}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v0

    iput v0, p0, Landroid/support/design/widget/f;->x:F

    iget v0, p0, Landroid/support/design/widget/f;->d:F

    iget v1, p0, Landroid/support/design/widget/f;->e:F

    iget-object v2, p0, Landroid/support/design/widget/f;->L:Landroid/view/animation/Interpolator;

    invoke-static {v0, v1, p1, v2}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v0

    invoke-direct {p0, v0}, Landroid/support/design/widget/f;->c(F)V

    iget-object v0, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    iget-object v1, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    if-eq v0, v1, :cond_1

    iget-object v0, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget-object v1, p0, Landroid/support/design/widget/f;->I:[I

    if-eqz v1, :cond_0

    iget-object v1, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    iget-object v2, p0, Landroid/support/design/widget/f;->I:[I

    const/4 v3, 0x0

    invoke-virtual {v1, v2, v3}, Landroid/content/res/ColorStateList;->getColorForState([II)I

    move-result v1

    goto :goto_0

    :cond_0
    iget-object v1, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    invoke-virtual {v1}, Landroid/content/res/ColorStateList;->getDefaultColor()I

    move-result v1

    :goto_0
    invoke-direct {p0}, Landroid/support/design/widget/f;->e()I

    move-result v2

    invoke-static {v1, v2, p1}, Landroid/support/design/widget/f;->a(IIF)I

    move-result v1

    goto :goto_1

    :cond_1
    iget-object v0, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-direct {p0}, Landroid/support/design/widget/f;->e()I

    move-result v1

    :goto_1
    invoke-virtual {v0, v1}, Landroid/text/TextPaint;->setColor(I)V

    iget-object v0, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget v1, p0, Landroid/support/design/widget/f;->Q:F

    iget v2, p0, Landroid/support/design/widget/f;->M:F

    const/4 v3, 0x0

    invoke-static {v1, v2, p1, v3}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v1

    iget v2, p0, Landroid/support/design/widget/f;->R:F

    iget v4, p0, Landroid/support/design/widget/f;->N:F

    invoke-static {v2, v4, p1, v3}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v2

    iget v4, p0, Landroid/support/design/widget/f;->S:F

    iget v5, p0, Landroid/support/design/widget/f;->O:F

    invoke-static {v4, v5, p1, v3}, Landroid/support/design/widget/f;->a(FFFLandroid/view/animation/Interpolator;)F

    move-result v3

    iget v4, p0, Landroid/support/design/widget/f;->T:I

    iget v5, p0, Landroid/support/design/widget/f;->P:I

    invoke-static {v4, v5, p1}, Landroid/support/design/widget/f;->a(IIF)I

    move-result p1

    invoke-virtual {v0, v1, v2, v3, p1}, Landroid/text/TextPaint;->setShadowLayer(FFFI)V

    iget-object p1, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-static {p1}, Landroid/support/v4/view/r;->c(Landroid/view/View;)V

    return-void
.end method

.method private c()V
    .locals 1

    iget-object v0, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    invoke-virtual {v0}, Landroid/graphics/Rect;->width()I

    move-result v0

    if-lez v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    invoke-virtual {v0}, Landroid/graphics/Rect;->height()I

    move-result v0

    if-lez v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-virtual {v0}, Landroid/graphics/Rect;->width()I

    move-result v0

    if-lez v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-virtual {v0}, Landroid/graphics/Rect;->height()I

    move-result v0

    if-lez v0, :cond_0

    const/4 v0, 0x1

    goto :goto_0

    :cond_0
    const/4 v0, 0x0

    :goto_0
    iput-boolean v0, p0, Landroid/support/design/widget/f;->n:Z

    return-void
.end method

.method private c(F)V
    .locals 8

    invoke-direct {p0, p1}, Landroid/support/design/widget/f;->d(F)V

    sget-boolean p1, Landroid/support/design/widget/f;->k:Z

    const/4 v0, 0x0

    if-eqz p1, :cond_0

    iget p1, p0, Landroid/support/design/widget/f;->G:F

    const/high16 v1, 0x3f800000    # 1.0f

    cmpl-float p1, p1, v1

    if-eqz p1, :cond_0

    const/4 p1, 0x1

    goto :goto_0

    :cond_0
    move p1, v0

    :goto_0
    iput-boolean p1, p0, Landroid/support/design/widget/f;->B:Z

    iget-boolean p1, p0, Landroid/support/design/widget/f;->B:Z

    if-eqz p1, :cond_3

    iget-object p1, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    if-nez p1, :cond_3

    iget-object p1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-virtual {p1}, Landroid/graphics/Rect;->isEmpty()Z

    move-result p1

    if-nez p1, :cond_3

    iget-object p1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-static {p1}, Landroid/text/TextUtils;->isEmpty(Ljava/lang/CharSequence;)Z

    move-result p1

    if-eqz p1, :cond_1

    goto :goto_1

    :cond_1
    const/4 p1, 0x0

    invoke-direct {p0, p1}, Landroid/support/design/widget/f;->b(F)V

    iget-object p1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {p1}, Landroid/text/TextPaint;->ascent()F

    move-result p1

    iput p1, p0, Landroid/support/design/widget/f;->E:F

    iget-object p1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {p1}, Landroid/text/TextPaint;->descent()F

    move-result p1

    iput p1, p0, Landroid/support/design/widget/f;->F:F

    iget-object p1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget-object v1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    iget-object v2, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-interface {v2}, Ljava/lang/CharSequence;->length()I

    move-result v2

    invoke-virtual {p1, v1, v0, v2}, Landroid/text/TextPaint;->measureText(Ljava/lang/CharSequence;II)F

    move-result p1

    invoke-static {p1}, Ljava/lang/Math;->round(F)I

    move-result p1

    iget v0, p0, Landroid/support/design/widget/f;->F:F

    iget v1, p0, Landroid/support/design/widget/f;->E:F

    sub-float/2addr v0, v1

    invoke-static {v0}, Ljava/lang/Math;->round(F)I

    move-result v0

    if-lez p1, :cond_3

    if-gtz v0, :cond_2

    goto :goto_1

    :cond_2
    sget-object v1, Landroid/graphics/Bitmap$Config;->ARGB_8888:Landroid/graphics/Bitmap$Config;

    invoke-static {p1, v0, v1}, Landroid/graphics/Bitmap;->createBitmap(IILandroid/graphics/Bitmap$Config;)Landroid/graphics/Bitmap;

    move-result-object p1

    iput-object p1, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    new-instance v1, Landroid/graphics/Canvas;

    iget-object p1, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    invoke-direct {v1, p1}, Landroid/graphics/Canvas;-><init>(Landroid/graphics/Bitmap;)V

    iget-object v2, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    const/4 v3, 0x0

    iget-object p1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-interface {p1}, Ljava/lang/CharSequence;->length()I

    move-result v4

    const/4 v5, 0x0

    int-to-float p1, v0

    iget-object v0, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v0}, Landroid/text/TextPaint;->descent()F

    move-result v0

    sub-float v6, p1, v0

    iget-object v7, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual/range {v1 .. v7}, Landroid/graphics/Canvas;->drawText(Ljava/lang/CharSequence;IIFFLandroid/graphics/Paint;)V

    iget-object p1, p0, Landroid/support/design/widget/f;->D:Landroid/graphics/Paint;

    if-nez p1, :cond_3

    new-instance p1, Landroid/graphics/Paint;

    const/4 v0, 0x3

    invoke-direct {p1, v0}, Landroid/graphics/Paint;-><init>(I)V

    iput-object p1, p0, Landroid/support/design/widget/f;->D:Landroid/graphics/Paint;

    :cond_3
    :goto_1
    iget-object p1, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-static {p1}, Landroid/support/v4/view/r;->c(Landroid/view/View;)V

    return-void
.end method

.method private d()V
    .locals 1

    iget v0, p0, Landroid/support/design/widget/f;->a:F

    invoke-direct {p0, v0}, Landroid/support/design/widget/f;->b(F)V

    return-void
.end method

.method private d(F)V
    .locals 8

    iget-object v0, p0, Landroid/support/design/widget/f;->i:Ljava/lang/CharSequence;

    if-nez v0, :cond_0

    return-void

    :cond_0
    iget-object v0, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    invoke-virtual {v0}, Landroid/graphics/Rect;->width()I

    move-result v0

    int-to-float v0, v0

    iget-object v1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-virtual {v1}, Landroid/graphics/Rect;->width()I

    move-result v1

    int-to-float v1, v1

    iget v2, p0, Landroid/support/design/widget/f;->e:F

    invoke-static {p1, v2}, Landroid/support/design/widget/f;->a(FF)Z

    move-result v2

    const/high16 v3, 0x3f800000    # 1.0f

    const/4 v4, 0x0

    const/4 v5, 0x1

    if-eqz v2, :cond_2

    iget p1, p0, Landroid/support/design/widget/f;->e:F

    iput v3, p0, Landroid/support/design/widget/f;->G:F

    iget-object v1, p0, Landroid/support/design/widget/f;->y:Landroid/graphics/Typeface;

    iget-object v2, p0, Landroid/support/design/widget/f;->g:Landroid/graphics/Typeface;

    invoke-static {v1, v2}, Landroid/support/design/widget/f;->a(Landroid/graphics/Typeface;Landroid/graphics/Typeface;)Z

    move-result v1

    if-eqz v1, :cond_1

    iget-object v1, p0, Landroid/support/design/widget/f;->g:Landroid/graphics/Typeface;

    iput-object v1, p0, Landroid/support/design/widget/f;->y:Landroid/graphics/Typeface;

    move v1, v5

    goto :goto_0

    :cond_1
    move v1, v4

    :goto_0
    move v2, p1

    move v6, v1

    goto :goto_3

    :cond_2
    iget v2, p0, Landroid/support/design/widget/f;->d:F

    iget-object v6, p0, Landroid/support/design/widget/f;->y:Landroid/graphics/Typeface;

    iget-object v7, p0, Landroid/support/design/widget/f;->h:Landroid/graphics/Typeface;

    invoke-static {v6, v7}, Landroid/support/design/widget/f;->a(Landroid/graphics/Typeface;Landroid/graphics/Typeface;)Z

    move-result v6

    if-eqz v6, :cond_3

    iget-object v6, p0, Landroid/support/design/widget/f;->h:Landroid/graphics/Typeface;

    iput-object v6, p0, Landroid/support/design/widget/f;->y:Landroid/graphics/Typeface;

    move v6, v5

    goto :goto_1

    :cond_3
    move v6, v4

    :goto_1
    iget v7, p0, Landroid/support/design/widget/f;->d:F

    invoke-static {p1, v7}, Landroid/support/design/widget/f;->a(FF)Z

    move-result v7

    if-eqz v7, :cond_4

    iput v3, p0, Landroid/support/design/widget/f;->G:F

    goto :goto_2

    :cond_4
    iget v7, p0, Landroid/support/design/widget/f;->d:F

    div-float/2addr p1, v7

    iput p1, p0, Landroid/support/design/widget/f;->G:F

    :goto_2
    iget p1, p0, Landroid/support/design/widget/f;->e:F

    iget v7, p0, Landroid/support/design/widget/f;->d:F

    div-float/2addr p1, v7

    mul-float v7, v1, p1

    cmpl-float v7, v7, v0

    if-lez v7, :cond_5

    div-float/2addr v0, p1

    invoke-static {v0, v1}, Ljava/lang/Math;->min(FF)F

    move-result v0

    goto :goto_3

    :cond_5
    move v0, v1

    :goto_3
    const/4 p1, 0x0

    cmpl-float p1, v0, p1

    if-lez p1, :cond_8

    iget p1, p0, Landroid/support/design/widget/f;->H:F

    cmpl-float p1, p1, v2

    if-nez p1, :cond_7

    iget-boolean p1, p0, Landroid/support/design/widget/f;->J:Z

    if-nez p1, :cond_7

    if-eqz v6, :cond_6

    goto :goto_4

    :cond_6
    move v6, v4

    goto :goto_5

    :cond_7
    :goto_4
    move v6, v5

    :goto_5
    iput v2, p0, Landroid/support/design/widget/f;->H:F

    iput-boolean v4, p0, Landroid/support/design/widget/f;->J:Z

    :cond_8
    iget-object p1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    if-eqz p1, :cond_9

    if-eqz v6, :cond_d

    :cond_9
    iget-object p1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget v1, p0, Landroid/support/design/widget/f;->H:F

    invoke-virtual {p1, v1}, Landroid/text/TextPaint;->setTextSize(F)V

    iget-object p1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget-object v1, p0, Landroid/support/design/widget/f;->y:Landroid/graphics/Typeface;

    invoke-virtual {p1, v1}, Landroid/text/TextPaint;->setTypeface(Landroid/graphics/Typeface;)Landroid/graphics/Typeface;

    iget-object p1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget v1, p0, Landroid/support/design/widget/f;->G:F

    cmpl-float v1, v1, v3

    if-eqz v1, :cond_a

    move v1, v5

    goto :goto_6

    :cond_a
    move v1, v4

    :goto_6
    invoke-virtual {p1, v1}, Landroid/text/TextPaint;->setLinearText(Z)V

    iget-object p1, p0, Landroid/support/design/widget/f;->i:Ljava/lang/CharSequence;

    iget-object v1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    sget-object v2, Landroid/text/TextUtils$TruncateAt;->END:Landroid/text/TextUtils$TruncateAt;

    invoke-static {p1, v1, v0, v2}, Landroid/text/TextUtils;->ellipsize(Ljava/lang/CharSequence;Landroid/text/TextPaint;FLandroid/text/TextUtils$TruncateAt;)Ljava/lang/CharSequence;

    move-result-object p1

    iget-object v0, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-static {p1, v0}, Landroid/text/TextUtils;->equals(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Z

    move-result v0

    if-nez v0, :cond_d

    iput-object p1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    iget-object p1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    iget-object v0, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-static {v0}, Landroid/support/v4/view/r;->e(Landroid/view/View;)I

    move-result v0

    if-ne v0, v5, :cond_b

    move v4, v5

    :cond_b
    if-eqz v4, :cond_c

    sget-object v0, Landroid/support/v4/f/c;->d:Landroid/support/v4/f/b;

    goto :goto_7

    :cond_c
    sget-object v0, Landroid/support/v4/f/c;->c:Landroid/support/v4/f/b;

    :goto_7
    invoke-interface {p1}, Ljava/lang/CharSequence;->length()I

    move-result v1

    invoke-interface {v0, p1, v1}, Landroid/support/v4/f/b;->a(Ljava/lang/CharSequence;I)Z

    move-result p1

    iput-boolean p1, p0, Landroid/support/design/widget/f;->A:Z

    :cond_d
    return-void
.end method

.method private e()I
    .locals 3

    iget-object v0, p0, Landroid/support/design/widget/f;->I:[I

    if-eqz v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    iget-object v1, p0, Landroid/support/design/widget/f;->I:[I

    const/4 v2, 0x0

    invoke-virtual {v0, v1, v2}, Landroid/content/res/ColorStateList;->getColorForState([II)I

    move-result v0

    return v0

    :cond_0
    iget-object v0, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    invoke-virtual {v0}, Landroid/content/res/ColorStateList;->getDefaultColor()I

    move-result v0

    return v0
.end method

.method private e(I)Landroid/graphics/Typeface;
    .locals 4

    iget-object v0, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-virtual {v0}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object v0

    const/4 v1, 0x1

    new-array v1, v1, [I

    const/4 v2, 0x0

    const v3, 0x10103ac

    aput v3, v1, v2

    invoke-virtual {v0, p1, v1}, Landroid/content/Context;->obtainStyledAttributes(I[I)Landroid/content/res/TypedArray;

    move-result-object p1

    :try_start_0
    invoke-virtual {p1, v2}, Landroid/content/res/TypedArray;->getString(I)Ljava/lang/String;

    move-result-object v0

    if-eqz v0, :cond_0

    invoke-static {v0, v2}, Landroid/graphics/Typeface;->create(Ljava/lang/String;I)Landroid/graphics/Typeface;

    move-result-object v0
    :try_end_0
    .catchall {:try_start_0 .. :try_end_0} :catchall_0

    invoke-virtual {p1}, Landroid/content/res/TypedArray;->recycle()V

    return-object v0

    :cond_0
    invoke-virtual {p1}, Landroid/content/res/TypedArray;->recycle()V

    const/4 p1, 0x0

    return-object p1

    :catchall_0
    move-exception v0

    invoke-virtual {p1}, Landroid/content/res/TypedArray;->recycle()V

    throw v0
.end method

.method private f()V
    .locals 1

    iget-object v0, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    if-eqz v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    invoke-virtual {v0}, Landroid/graphics/Bitmap;->recycle()V

    const/4 v0, 0x0

    iput-object v0, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    :cond_0
    return-void
.end method


# virtual methods
.method final a()Landroid/graphics/Typeface;
    .locals 1

    iget-object v0, p0, Landroid/support/design/widget/f;->g:Landroid/graphics/Typeface;

    if-eqz v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->g:Landroid/graphics/Typeface;

    return-object v0

    :cond_0
    sget-object v0, Landroid/graphics/Typeface;->DEFAULT:Landroid/graphics/Typeface;

    return-object v0
.end method

.method final a(F)V
    .locals 2

    const/4 v0, 0x0

    const/high16 v1, 0x3f800000    # 1.0f

    invoke-static {p1, v0, v1}, Landroid/support/v4/c/a;->a(FFF)F

    move-result p1

    iget v0, p0, Landroid/support/design/widget/f;->a:F

    cmpl-float v0, p1, v0

    if-eqz v0, :cond_0

    iput p1, p0, Landroid/support/design/widget/f;->a:F

    invoke-direct {p0}, Landroid/support/design/widget/f;->d()V

    :cond_0
    return-void
.end method

.method final a(I)V
    .locals 1

    iget v0, p0, Landroid/support/design/widget/f;->b:I

    if-eq v0, p1, :cond_0

    iput p1, p0, Landroid/support/design/widget/f;->b:I

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    :cond_0
    return-void
.end method

.method final a(IIII)V
    .locals 1

    iget-object v0, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-static {v0, p1, p2, p3, p4}, Landroid/support/design/widget/f;->a(Landroid/graphics/Rect;IIII)Z

    move-result v0

    if-nez v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-virtual {v0, p1, p2, p3, p4}, Landroid/graphics/Rect;->set(IIII)V

    const/4 p1, 0x1

    iput-boolean p1, p0, Landroid/support/design/widget/f;->J:Z

    invoke-direct {p0}, Landroid/support/design/widget/f;->c()V

    :cond_0
    return-void
.end method

.method final a(Landroid/content/res/ColorStateList;)V
    .locals 1

    iget-object v0, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    if-eq v0, p1, :cond_0

    iput-object p1, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    :cond_0
    return-void
.end method

.method public final a(Landroid/graphics/Canvas;)V
    .locals 9

    invoke-virtual {p1}, Landroid/graphics/Canvas;->save()I

    move-result v0

    iget-object v1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    if-eqz v1, :cond_5

    iget-boolean v1, p0, Landroid/support/design/widget/f;->n:Z

    if-eqz v1, :cond_5

    iget v6, p0, Landroid/support/design/widget/f;->w:F

    iget v1, p0, Landroid/support/design/widget/f;->x:F

    iget-boolean v2, p0, Landroid/support/design/widget/f;->B:Z

    if-eqz v2, :cond_0

    iget-object v2, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    if-eqz v2, :cond_0

    const/4 v2, 0x1

    goto :goto_0

    :cond_0
    const/4 v2, 0x0

    :goto_0
    if-eqz v2, :cond_1

    iget v3, p0, Landroid/support/design/widget/f;->E:F

    iget v4, p0, Landroid/support/design/widget/f;->G:F

    mul-float/2addr v3, v4

    goto :goto_1

    :cond_1
    iget-object v3, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v3}, Landroid/text/TextPaint;->ascent()F

    const/4 v3, 0x0

    iget-object v4, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v4}, Landroid/text/TextPaint;->descent()F

    :goto_1
    if-eqz v2, :cond_2

    add-float/2addr v1, v3

    :cond_2
    move v7, v1

    iget v1, p0, Landroid/support/design/widget/f;->G:F

    const/high16 v3, 0x3f800000    # 1.0f

    cmpl-float v1, v1, v3

    if-eqz v1, :cond_3

    iget v1, p0, Landroid/support/design/widget/f;->G:F

    iget v3, p0, Landroid/support/design/widget/f;->G:F

    invoke-virtual {p1, v1, v3, v6, v7}, Landroid/graphics/Canvas;->scale(FFFF)V

    :cond_3
    if-eqz v2, :cond_4

    iget-object v1, p0, Landroid/support/design/widget/f;->C:Landroid/graphics/Bitmap;

    iget-object v2, p0, Landroid/support/design/widget/f;->D:Landroid/graphics/Paint;

    invoke-virtual {p1, v1, v6, v7, v2}, Landroid/graphics/Canvas;->drawBitmap(Landroid/graphics/Bitmap;FFLandroid/graphics/Paint;)V

    goto :goto_2

    :cond_4
    iget-object v3, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    const/4 v4, 0x0

    iget-object v1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-interface {v1}, Ljava/lang/CharSequence;->length()I

    move-result v5

    iget-object v8, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    move-object v2, p1

    invoke-virtual/range {v2 .. v8}, Landroid/graphics/Canvas;->drawText(Ljava/lang/CharSequence;IIFFLandroid/graphics/Paint;)V

    :cond_5
    :goto_2
    invoke-virtual {p1, v0}, Landroid/graphics/Canvas;->restoreToCount(I)V

    return-void
.end method

.method final a(Landroid/graphics/Typeface;)V
    .locals 0

    iput-object p1, p0, Landroid/support/design/widget/f;->h:Landroid/graphics/Typeface;

    iput-object p1, p0, Landroid/support/design/widget/f;->g:Landroid/graphics/Typeface;

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    return-void
.end method

.method final a(Landroid/view/animation/Interpolator;)V
    .locals 0

    iput-object p1, p0, Landroid/support/design/widget/f;->L:Landroid/view/animation/Interpolator;

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    return-void
.end method

.method final a(Ljava/lang/CharSequence;)V
    .locals 1

    if-eqz p1, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->i:Ljava/lang/CharSequence;

    invoke-virtual {p1, v0}, Ljava/lang/Object;->equals(Ljava/lang/Object;)Z

    move-result v0

    if-nez v0, :cond_1

    :cond_0
    iput-object p1, p0, Landroid/support/design/widget/f;->i:Ljava/lang/CharSequence;

    const/4 p1, 0x0

    iput-object p1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-direct {p0}, Landroid/support/design/widget/f;->f()V

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    :cond_1
    return-void
.end method

.method final a([I)Z
    .locals 2

    iput-object p1, p0, Landroid/support/design/widget/f;->I:[I

    iget-object p1, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    const/4 v0, 0x0

    const/4 v1, 0x1

    if-eqz p1, :cond_0

    iget-object p1, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    invoke-virtual {p1}, Landroid/content/res/ColorStateList;->isStateful()Z

    move-result p1

    if-nez p1, :cond_1

    :cond_0
    iget-object p1, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    if-eqz p1, :cond_2

    iget-object p1, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    invoke-virtual {p1}, Landroid/content/res/ColorStateList;->isStateful()Z

    move-result p1

    if-eqz p1, :cond_2

    :cond_1
    move p1, v1

    goto :goto_0

    :cond_2
    move p1, v0

    :goto_0
    if-eqz p1, :cond_3

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    return v1

    :cond_3
    return v0
.end method

.method public final b()V
    .locals 11

    iget-object v0, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-virtual {v0}, Landroid/view/View;->getHeight()I

    move-result v0

    if-lez v0, :cond_a

    iget-object v0, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-virtual {v0}, Landroid/view/View;->getWidth()I

    move-result v0

    if-lez v0, :cond_a

    iget v0, p0, Landroid/support/design/widget/f;->H:F

    iget v1, p0, Landroid/support/design/widget/f;->e:F

    invoke-direct {p0, v1}, Landroid/support/design/widget/f;->d(F)V

    iget-object v1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    const/4 v2, 0x0

    const/4 v3, 0x0

    if-eqz v1, :cond_0

    iget-object v1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget-object v4, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    iget-object v5, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-interface {v5}, Ljava/lang/CharSequence;->length()I

    move-result v5

    invoke-virtual {v1, v4, v3, v5}, Landroid/text/TextPaint;->measureText(Ljava/lang/CharSequence;II)F

    move-result v1

    goto :goto_0

    :cond_0
    move v1, v2

    :goto_0
    iget v4, p0, Landroid/support/design/widget/f;->c:I

    iget-boolean v5, p0, Landroid/support/design/widget/f;->A:Z

    invoke-static {v4, v5}, Landroid/support/v4/view/e;->a(II)I

    move-result v4

    and-int/lit8 v5, v4, 0x70

    const/16 v6, 0x50

    const/16 v7, 0x30

    const/high16 v8, 0x40000000    # 2.0f

    if-eq v5, v7, :cond_2

    if-eq v5, v6, :cond_1

    iget-object v5, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v5}, Landroid/text/TextPaint;->descent()F

    move-result v5

    iget-object v9, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v9}, Landroid/text/TextPaint;->ascent()F

    move-result v9

    sub-float/2addr v5, v9

    div-float/2addr v5, v8

    iget-object v9, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v9}, Landroid/text/TextPaint;->descent()F

    move-result v9

    sub-float/2addr v5, v9

    iget-object v9, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    invoke-virtual {v9}, Landroid/graphics/Rect;->centerY()I

    move-result v9

    int-to-float v9, v9

    add-float/2addr v9, v5

    iput v9, p0, Landroid/support/design/widget/f;->t:F

    goto :goto_2

    :cond_1
    iget-object v5, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    iget v5, v5, Landroid/graphics/Rect;->bottom:I

    int-to-float v5, v5

    goto :goto_1

    :cond_2
    iget-object v5, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    iget v5, v5, Landroid/graphics/Rect;->top:I

    int-to-float v5, v5

    iget-object v9, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v9}, Landroid/text/TextPaint;->ascent()F

    move-result v9

    sub-float/2addr v5, v9

    :goto_1
    iput v5, p0, Landroid/support/design/widget/f;->t:F

    :goto_2
    const v5, 0x800007

    and-int/2addr v4, v5

    const/4 v9, 0x5

    const/4 v10, 0x1

    if-eq v4, v10, :cond_4

    if-eq v4, v9, :cond_3

    iget-object v1, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    iget v1, v1, Landroid/graphics/Rect;->left:I

    int-to-float v1, v1

    iput v1, p0, Landroid/support/design/widget/f;->v:F

    goto :goto_4

    :cond_3
    iget-object v4, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    iget v4, v4, Landroid/graphics/Rect;->right:I

    int-to-float v4, v4

    goto :goto_3

    :cond_4
    iget-object v4, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    invoke-virtual {v4}, Landroid/graphics/Rect;->centerX()I

    move-result v4

    int-to-float v4, v4

    div-float/2addr v1, v8

    :goto_3
    sub-float/2addr v4, v1

    iput v4, p0, Landroid/support/design/widget/f;->v:F

    :goto_4
    iget v1, p0, Landroid/support/design/widget/f;->d:F

    invoke-direct {p0, v1}, Landroid/support/design/widget/f;->d(F)V

    iget-object v1, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    if-eqz v1, :cond_5

    iget-object v1, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    iget-object v2, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    iget-object v4, p0, Landroid/support/design/widget/f;->z:Ljava/lang/CharSequence;

    invoke-interface {v4}, Ljava/lang/CharSequence;->length()I

    move-result v4

    invoke-virtual {v1, v2, v3, v4}, Landroid/text/TextPaint;->measureText(Ljava/lang/CharSequence;II)F

    move-result v2

    :cond_5
    iget v1, p0, Landroid/support/design/widget/f;->b:I

    iget-boolean v3, p0, Landroid/support/design/widget/f;->A:Z

    invoke-static {v1, v3}, Landroid/support/v4/view/e;->a(II)I

    move-result v1

    and-int/lit8 v3, v1, 0x70

    if-eq v3, v7, :cond_7

    if-eq v3, v6, :cond_6

    iget-object v3, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v3}, Landroid/text/TextPaint;->descent()F

    move-result v3

    iget-object v4, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v4}, Landroid/text/TextPaint;->ascent()F

    move-result v4

    sub-float/2addr v3, v4

    div-float/2addr v3, v8

    iget-object v4, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v4}, Landroid/text/TextPaint;->descent()F

    move-result v4

    sub-float/2addr v3, v4

    iget-object v4, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-virtual {v4}, Landroid/graphics/Rect;->centerY()I

    move-result v4

    int-to-float v4, v4

    add-float/2addr v4, v3

    iput v4, p0, Landroid/support/design/widget/f;->s:F

    goto :goto_6

    :cond_6
    iget-object v3, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    iget v3, v3, Landroid/graphics/Rect;->bottom:I

    int-to-float v3, v3

    goto :goto_5

    :cond_7
    iget-object v3, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    iget v3, v3, Landroid/graphics/Rect;->top:I

    int-to-float v3, v3

    iget-object v4, p0, Landroid/support/design/widget/f;->K:Landroid/text/TextPaint;

    invoke-virtual {v4}, Landroid/text/TextPaint;->ascent()F

    move-result v4

    sub-float/2addr v3, v4

    :goto_5
    iput v3, p0, Landroid/support/design/widget/f;->s:F

    :goto_6
    and-int/2addr v1, v5

    if-eq v1, v10, :cond_9

    if-eq v1, v9, :cond_8

    iget-object v1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    iget v1, v1, Landroid/graphics/Rect;->left:I

    int-to-float v1, v1

    :goto_7
    iput v1, p0, Landroid/support/design/widget/f;->u:F

    goto :goto_9

    :cond_8
    iget-object v1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    iget v1, v1, Landroid/graphics/Rect;->right:I

    int-to-float v1, v1

    goto :goto_8

    :cond_9
    iget-object v1, p0, Landroid/support/design/widget/f;->o:Landroid/graphics/Rect;

    invoke-virtual {v1}, Landroid/graphics/Rect;->centerX()I

    move-result v1

    int-to-float v1, v1

    div-float/2addr v2, v8

    :goto_8
    sub-float/2addr v1, v2

    goto :goto_7

    :goto_9
    invoke-direct {p0}, Landroid/support/design/widget/f;->f()V

    invoke-direct {p0, v0}, Landroid/support/design/widget/f;->c(F)V

    invoke-direct {p0}, Landroid/support/design/widget/f;->d()V

    :cond_a
    return-void
.end method

.method final b(I)V
    .locals 1

    iget v0, p0, Landroid/support/design/widget/f;->c:I

    if-eq v0, p1, :cond_0

    iput p1, p0, Landroid/support/design/widget/f;->c:I

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    :cond_0
    return-void
.end method

.method final b(IIII)V
    .locals 1

    iget-object v0, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    invoke-static {v0, p1, p2, p3, p4}, Landroid/support/design/widget/f;->a(Landroid/graphics/Rect;IIII)Z

    move-result v0

    if-nez v0, :cond_0

    iget-object v0, p0, Landroid/support/design/widget/f;->p:Landroid/graphics/Rect;

    invoke-virtual {v0, p1, p2, p3, p4}, Landroid/graphics/Rect;->set(IIII)V

    const/4 p1, 0x1

    iput-boolean p1, p0, Landroid/support/design/widget/f;->J:Z

    invoke-direct {p0}, Landroid/support/design/widget/f;->c()V

    :cond_0
    return-void
.end method

.method final b(Landroid/content/res/ColorStateList;)V
    .locals 1

    iget-object v0, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    if-eq v0, p1, :cond_0

    iput-object p1, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    :cond_0
    return-void
.end method

.method final c(I)V
    .locals 3

    iget-object v0, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-virtual {v0}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object v0

    sget-object v1, Landroid/support/v7/a/a$j;->TextAppearance:[I

    invoke-static {v0, p1, v1}, Landroid/support/v7/widget/bs;->a(Landroid/content/Context;I[I)Landroid/support/v7/widget/bs;

    move-result-object v0

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textColor:I

    invoke-virtual {v0, v1}, Landroid/support/v7/widget/bs;->f(I)Z

    move-result v1

    if-eqz v1, :cond_0

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textColor:I

    invoke-virtual {v0, v1}, Landroid/support/v7/widget/bs;->e(I)Landroid/content/res/ColorStateList;

    move-result-object v1

    iput-object v1, p0, Landroid/support/design/widget/f;->f:Landroid/content/res/ColorStateList;

    :cond_0
    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textSize:I

    invoke-virtual {v0, v1}, Landroid/support/v7/widget/bs;->f(I)Z

    move-result v1

    if-eqz v1, :cond_1

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textSize:I

    iget v2, p0, Landroid/support/design/widget/f;->e:F

    float-to-int v2, v2

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->e(II)I

    move-result v1

    int-to-float v1, v1

    iput v1, p0, Landroid/support/design/widget/f;->e:F

    :cond_1
    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowColor:I

    const/4 v2, 0x0

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(II)I

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->P:I

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowDx:I

    const/4 v2, 0x0

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(IF)F

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->N:F

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowDy:I

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(IF)F

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->O:F

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowRadius:I

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(IF)F

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->M:F

    iget-object v0, v0, Landroid/support/v7/widget/bs;->b:Landroid/content/res/TypedArray;

    invoke-virtual {v0}, Landroid/content/res/TypedArray;->recycle()V

    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v1, 0x10

    if-lt v0, v1, :cond_2

    invoke-direct {p0, p1}, Landroid/support/design/widget/f;->e(I)Landroid/graphics/Typeface;

    move-result-object p1

    iput-object p1, p0, Landroid/support/design/widget/f;->g:Landroid/graphics/Typeface;

    :cond_2
    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    return-void
.end method

.method final d(I)V
    .locals 3

    iget-object v0, p0, Landroid/support/design/widget/f;->m:Landroid/view/View;

    invoke-virtual {v0}, Landroid/view/View;->getContext()Landroid/content/Context;

    move-result-object v0

    sget-object v1, Landroid/support/v7/a/a$j;->TextAppearance:[I

    invoke-static {v0, p1, v1}, Landroid/support/v7/widget/bs;->a(Landroid/content/Context;I[I)Landroid/support/v7/widget/bs;

    move-result-object v0

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textColor:I

    invoke-virtual {v0, v1}, Landroid/support/v7/widget/bs;->f(I)Z

    move-result v1

    if-eqz v1, :cond_0

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textColor:I

    invoke-virtual {v0, v1}, Landroid/support/v7/widget/bs;->e(I)Landroid/content/res/ColorStateList;

    move-result-object v1

    iput-object v1, p0, Landroid/support/design/widget/f;->r:Landroid/content/res/ColorStateList;

    :cond_0
    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textSize:I

    invoke-virtual {v0, v1}, Landroid/support/v7/widget/bs;->f(I)Z

    move-result v1

    if-eqz v1, :cond_1

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_textSize:I

    iget v2, p0, Landroid/support/design/widget/f;->d:F

    float-to-int v2, v2

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->e(II)I

    move-result v1

    int-to-float v1, v1

    iput v1, p0, Landroid/support/design/widget/f;->d:F

    :cond_1
    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowColor:I

    const/4 v2, 0x0

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(II)I

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->T:I

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowDx:I

    const/4 v2, 0x0

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(IF)F

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->R:F

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowDy:I

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(IF)F

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->S:F

    sget v1, Landroid/support/v7/a/a$j;->TextAppearance_android_shadowRadius:I

    invoke-virtual {v0, v1, v2}, Landroid/support/v7/widget/bs;->a(IF)F

    move-result v1

    iput v1, p0, Landroid/support/design/widget/f;->Q:F

    iget-object v0, v0, Landroid/support/v7/widget/bs;->b:Landroid/content/res/TypedArray;

    invoke-virtual {v0}, Landroid/content/res/TypedArray;->recycle()V

    sget v0, Landroid/os/Build$VERSION;->SDK_INT:I

    const/16 v1, 0x10

    if-lt v0, v1, :cond_2

    invoke-direct {p0, p1}, Landroid/support/design/widget/f;->e(I)Landroid/graphics/Typeface;

    move-result-object p1

    iput-object p1, p0, Landroid/support/design/widget/f;->h:Landroid/graphics/Typeface;

    :cond_2
    invoke-virtual {p0}, Landroid/support/design/widget/f;->b()V

    return-void
.end method
